...
class HDPoolWebsocketApi {
  ...
  async init() {
    ...
    this.client.addEventListener('close', () => this.publish('websocket/closed'));
    this.client.addEventListener('open', () => this.publish('websocket/opened'));

    this.updateConnectionAliveInterval = setInterval(this.updateConnectionAlive.bind(this), 30 * 1000);
  }

  async updateConnectionAlive() {
    if (this.connecting || this.lastHeartbeat.isAfter(dayjs().subtract(1, 'minute'))) {
      return;
    }
