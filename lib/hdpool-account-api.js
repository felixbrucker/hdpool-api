...
class HDPoolAccountApi extends HDPoolWebsocketApi {
  ...
  constructor(userId, sessionKey, useEcoPool = false) {
    super();
    if (!userId) {
    ...
    this.websocketEndpoints = useEcoPool ? [
      `wss://eco.hdpool.com/?uid=${this.uid}&key=${this.key}`,
    ] : [
  ...
  async cancelRentingOrder(rentingOrderId, reason = 'wrong amount') {
    if (!rentingOrderId) {
      throw new Error('No rentingOrderId supplied!');
    }

    await this.sendMessageAndAwaitResponse({
      chk: new Date().getTime(),
      cmd: 'apid.cancel_rentin_order',
      para: {
        uid: this.uid,
        type: 'bhd',
        reject: reason,
        rentin_id: rentingOrderId,
      },
    });
  }
  ...
  async withdraw(amount, emailCode = null, authCode = null) {
    ...
    await this.sendMessageAndAwaitResponse({
      ...
      para: {
        ...
        email_code: emailCode ? emailCode : '',
      },
    });
  }
