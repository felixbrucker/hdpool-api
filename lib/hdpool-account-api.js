...
class HDPoolAccountApi extends HDPoolWebsocketApi {
  ...
  async getNextFreePaymentDate() {
    ...
    const nonCanceledWithdraws = withdrawHistory.filter(withdraw => withdraw.status !== 3);

    return dayjs(nonCanceledWithdraws[0].create_ts).add(7, 'day').add(5, 'minute').toDate();
  }
