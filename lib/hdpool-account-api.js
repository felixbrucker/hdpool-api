...
class HDPoolAccountApi extends HDPoolWebsocketApi {
  ...
  async init() {
    ...
    this.client.addEventListener('message', this.onBestMiningInfoRaw);
  }
  ...
  destroy(complete = true) {
    if (this.client) {
      this.client.removeEventListener('message', this.onBestMiningInfoRaw);
    }
